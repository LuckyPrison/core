...
class Warn extends TimedPunishment {
	...
	public static Punishment fromDocument(Document document)
	{
		...
		return new Warn(id, holder, punisher, reason, creation, expiry, updater, referenced);
	}

	Warn(int id, PunishmentHolder holder, Punisher punisher, String reason, Instant placed, Instant expiry, Punisher updater, int[] referenced)
	{
		super(id, PunishmentType.WARN, holder, punisher, reason, placed, expiry, updater, referenced);
	}
